...
export default function SearchPage() {
  ...
  const handleFileClick = (path, line) => {
    ...
    const projectParam =
      selectedProjects.length > 0 ? `&project_id=${selectedProjects[0]}` : "";
    navigate(
      `/files/${encodeURIComponent(path.trim())}?line=${lineParam}${projectParam}`,
    );
  };
                ...
                {hasActiveFilters && (
                  <div className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center font-bold">
                    {[
                      ...
                      filters.fileType,
                      filters.symbolType,
                    ].filter(Boolean).length + selectedProjects.length}
                  </div>
                )}
              </button>
            </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      Select which projects to search
                    </p>
                  </div>
                </div>
