...
export default function ProjectChatPage() {
  ...
  const { config, trackPerformance } = useAIConfig();
  ...
  const { addToCitations, clearCitations, currentContext, analyzeMessage } =
    useKnowledgeContext();
  ...
  const handleSendMessage = useCallback(
    async (content, metadata = {}) => {
      ...
      if (currentContext.length) {
        fullMeta.referenced_chunks = currentContext.map(
          (c) => c.id || c.source,
        );
        fullMeta.referenced_files = [
          ...new Set(
            currentContext.map((c) => c.source || c.file_path).filter(Boolean),
          ),
        ];
      }
  ...
  const handleInteractiveElement = useCallback(async (elementId, result) => {
    try {
      ...
      toast.error(
        `Interactive element failed: ${error.message || "Unknown error"}`,
      );
      return {
        success: false,
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("contextAction", handleContextAction);
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-500">
              {otherUsersTyping.length === 1
                ? "User is typing..."
                : `${otherUsersTyping.length} users are typing...`}
            </span>
          </div>
        </div>
      </div>
        ...
        >
          ...
          {streaming ? (
            ...
            <EnhancedMessageRenderer
              ...
              onCodeRun={(code, lang) =>
                handleCodeExecution(code, lang, msg.id)
              }
