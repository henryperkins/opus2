...
export const defaultChatSettings = {
  ...
  models: {
    ...
    autoSwitch: true,
    costLimit: 0.1, // dollars per request
  },
  rendering: {
...
export const validateChatSettings = (settings) => {
  ...
  if (
    settings.knowledge?.maxContextDocs < 1 ||
    settings.knowledge?.maxContextDocs > 50
  ) {
    errors.push("maxContextDocs must be between 1 and 50");
  }

  if (
    settings.knowledge?.minConfidence < 0 ||
    settings.knowledge?.minConfidence > 1
  ) {
  ...
  if (
    settings.models?.costLimit &&
    (settings.models.costLimit < 0 || settings.models.costLimit > 10)
  ) {
  ...
  if (
    settings.rendering?.syntaxTheme &&
    !validSyntaxThemes.includes(settings.rendering.syntaxTheme)
  ) {
