...
const SessionRAGBadge = ({ messages = [], className = "" }) => {
  ...
  const ragStats = React.useMemo(() => {
    ...
    const ragActiveMessages = assistantMessages.filter(
      (msg) => msg.metadata?.ragUsed === true,
    );

    // Calculate total confidence only from messages where RAG was actually used
    const totalConfidence = ragActiveMessages.reduce(
      (sum, msg) => sum + (msg.metadata?.ragConfidence || 0),
      0,
    );

    const avgConfidence =
      ragActiveMessages.length > 0
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-sm font-medium ${statusConfig.bgColor} ${statusConfig.textColor} ${statusConfig.borderColor} ${className}`}
    >
