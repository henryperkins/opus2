...
export default function ThinkingModeSelector({
  ...
  const selectedMode =
    THINKING_MODES.find((mode) => mode.id === value) || THINKING_MODES[0];
  const selectedDepth =
    DEPTH_LEVELS.find((level) => level.id === depth) || DEPTH_LEVELS[1];
  const SelectedIcon = selectedMode.icon;

  const isClaudeProvider = config?.current?.provider === "anthropic";
  const claudeThinkingEnabled =
    config?.current?.claude_extended_thinking && isClaudeProvider;
                ...
                return (
                  ...
                  >
                    <div className={`p-1 rounded bg-${mode.color}-100`}>
                      <IconComponent
                        className={`h-4 w-4 text-${mode.color}-600`}
                      />
                    </div>
