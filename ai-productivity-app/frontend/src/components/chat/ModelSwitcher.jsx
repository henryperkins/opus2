...
export default function ModelSwitcher({
  ...
  const { currentModel, setModel, modelStats, isLoading, testModel } =
    useModelSelection();
    ...
    if (config?.providers) {
      ...
      if (config.providers.openai?.chat_models) {
        config.providers.openai.chat_models.forEach((modelId) => {
          const defaultModel = defaultModelOptions.find(
            (m) => m.id === modelId,
          );
          if (defaultModel) {
  ...
  if (compact) {
    return (
      <div className="relative" ref={dropdownRef}>
        ...
        >
          <span className="font-medium">{config?.model_idInfo.name}</span>
          <ChevronDown
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        >
          <div className="flex items-center space-x-3">
            {getQualityIcon(config?.model_idInfo.quality)}
            <div className="text-left">
              <div className="font-medium text-gray-900">
                {config?.model_idInfo.name}
              </div>
              <div className="text-xs text-gray-500">
                {config?.model_idInfo.contextLength.toLocaleString()} tokens •{" "}
                {config?.model_idInfo.provider}
              </div>
            </div>
          </div>
          <ChevronDown
        ...
        <div className="mt-3 flex items-center justify-between text-xs text-gray-500">
          ...
          <span
            className={`font-medium ${
              modelStats.averageLatency < 2000
                ? "text-green-600"
                : modelStats.averageLatency < 5000
                  ? "text-yellow-600"
                  : "text-red-600"
            }`}
          >
