...
const RAGStatusIndicator = ({
  ...
  if (
    !ragUsed &&
    status !== RAG_STATUS_TYPES.ERROR &&
    status !== RAG_STATUS_TYPES.INACTIVE
  ) {
    return null;
  }
  ...
  const getStatusText = () => {
    switch (status) {
      case RAG_STATUS_TYPES.ACTIVE:
        return ragUsed
          ? `RAG Active (${sourcesCount} source${sourcesCount !== 1 ? "s" : ""})`
          : "RAG Active";
      case RAG_STATUS_TYPES.DEGRADED:
        return "RAG Degraded";
      case RAG_STATUS_TYPES.POOR:
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-1">
        <div
          className={`w-2 h-2 rounded-full ${ragUsed ? "bg-green-500" : "bg-gray-400"}`}
        />
  ...
  return (
    <div
      className={`flex items-center gap-2 px-3 py-1.5 rounded-md border text-sm ${getStatusColor()}`}
    >
