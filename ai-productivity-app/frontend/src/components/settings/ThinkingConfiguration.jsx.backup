...
import { Brain, Zap, Target, GitBranch, ArrowDownUp, Search, RotateCcw, Info, CheckCircle, XCircle, Settings, AlertTriangle } from 'lucide-react';
...
const THINKING_DEPTHS = [
  { id: 'surface', name: 'Surface', description: 'Quick overview and key points' },
  { id: 'detailed', name: 'Detailed', description: 'Thorough analysis with examples' },
  { id: 'comprehensive', name: 'Comprehensive', description: 'In-depth coverage of all aspects' },
  { id: 'exhaustive', name: 'Exhaustive', description: 'Complete analysis of every angle' }
];
...
const CLAUDE_THINKING_MODES = [
  ...
  { id: 'aggressive', name: 'Aggressive', description: 'Maximum thinking depth' }
];

export default function ThinkingConfiguration() {
  const { config, updateConfig, loading } = useConfigOptimized();
  ...
  useEffect(() => {
    if (config?.current) {
      setThinkingConfig(prev => ({
        ...prev,
        claude_extended_thinking: config.current.claude_extended_thinking ?? prev.claude_extended_thinking,
        claude_thinking_mode: config.current.claude_thinking_mode ?? prev.claude_thinking_mode,
        claude_thinking_budget_tokens: config.current.claude_thinking_budget_tokens ?? prev.claude_thinking_budget_tokens,
        claude_show_thinking_process: config.current.claude_show_thinking_process ?? prev.claude_show_thinking_process,
        claude_adaptive_thinking_budget: config.current.claude_adaptive_thinking_budget ?? prev.claude_adaptive_thinking_budget,
        claude_max_thinking_budget: config.current.claude_max_thinking_budget ?? prev.claude_max_thinking_budget,
        enable_reasoning: config.current.enable_reasoning ?? prev.enable_reasoning,
        reasoning_effort: config.current.reasoning_effort ?? prev.reasoning_effort
      }));
    }
  }, [config]);
  ...
  const testThinkingMode = async (mode) => {
    ...
    try {
      const testTask = "Analyze the pros and cons of using microservices architecture for a medium-sized web application";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-xl font-semibold text-gray-900">Thinking & Reasoning Configuration</h2>
      </div>

      {/* Provider-specific thinking settings */}
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Provider-Specific Settings</h3>
        ...
        {isClaudeProvider() && (
          <div className="space-y-4">
            <h4 className="font-medium text-gray-700">Claude Extended Thinking</h4>
            
            {/* Enable/Disable Claude Thinking */}
            <div className="flex items-center justify-between">
              <div>
                <label className="text-sm font-medium text-gray-700">Extended Thinking</label>
                <p className="text-xs text-gray-500">Enable Claude's transparent reasoning process</p>
              </div>
              ...
              <>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Thinking Budget: {thinkingConfig.claude_thinking_budget_tokens} tokens
                  </label>
                  <input
                    type="range"
                    min="1024"
                    ...
                    onChange={(e) => handleConfigChange('claude_thinking_budget_tokens', parseInt(e.target.value))}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">Display Claude's reasoning steps in responses</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={thinkingConfig.claude_show_thinking_process}
                      onChange={(e) => handleConfigChange('claude_show_thinking_process', e.target.checked)}
                      className="sr-only peer"
                    />
                    <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                  </label>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <label className="text-sm font-medium text-gray-700">Adaptive Budget</label>
                    <p className="text-xs text-gray-500">Automatically adjust thinking budget based on task complexity</p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={thinkingConfig.claude_adaptive_thinking_budget}
                      onChange={(e) => handleConfigChange('claude_adaptive_thinking_budget', e.target.checked)}
                      className="sr-only peer"
                    />
                    <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                  </label>
        ...
        {isAzureOrOpenAI() && (
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                <label className="text-sm font-medium text-gray-700">Enable Reasoning</label>
                <p className="text-xs text-gray-500">Use Azure Responses API reasoning features</p>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  checked={thinkingConfig.enable_reasoning}
                  onChange={(e) => handleConfigChange('enable_reasoning', e.target.checked)}
                  className="sr-only peer"
                />
                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
              </label>
      ...
      {/* Thinking Modes */}
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Comprehensive Analysis Modes</h3>
          ...
          {THINKING_MODES.map((mode) => {
            ...
            return (
              <div
                ...
                onClick={() => handleConfigChange('default_thinking_mode', mode.id)}
              >
        ...
        {/* Thinking Depth */}
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">Default Analysis Depth</label>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {THINKING_DEPTHS.map(depth => (
              <button
                key={depth.id}
                onClick={() => handleConfigChange('default_thinking_depth', depth.id)}
              ...
              >
                ...
                <div className="text-xs text-gray-500 mt-1">{depth.description}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <label className="text-sm font-medium text-gray-700">Auto-select Thinking Mode</label>
              <p className="text-xs text-gray-500">Automatically choose the best thinking approach for each task</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={thinkingConfig.auto_select_thinking_mode}
                onChange={(e) => handleConfigChange('auto_select_thinking_mode', e.target.checked)}
                className="sr-only peer"
              />
              <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
          ...
          <div className="flex items-center justify-between">
            <div>
              <label className="text-sm font-medium text-gray-700">Enable Documentation Fetching</label>
              <p className="text-xs text-gray-500">Allow AI to fetch and analyze external documentation</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={thinkingConfig.enable_documentation_fetching}
                onChange={(e) => handleConfigChange('enable_documentation_fetching', e.target.checked)}
                className="sr-only peer"
              />
              <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
            </label>
