  ...
  const uploadFiles = useCallback(async () => {
    ...
    try {
      ...
      const response = await client.post(
        `/api/knowledge/projects/${projectId}/upload`,
        formData,
        {
          headers: {
        ...
        setFiles((prevFiles) =>
          prevFiles.map((fileItem) => {
            const uploadResult = result.results?.find(
              (r) => r.file === fileItem.name || r.title === fileItem.name,
            );
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="space-y-2">
          <Upload
            className={`w-8 h-8 mx-auto ${disabled ? "text-gray-400" : "text-gray-500"}`}
          />
          <div>
            <p
                  ...
                  <div className="flex items-center space-x-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500">
                        ...
                        {ALLOWED_FILE_TYPES[fileItem.type]?.label || "Unknown"}
                      </p>
                    </div>
                  </div>
              ...
              >
                ...
                {result.existing_title && (
                  <div className="text-xs opacity-75">
                    Existing: {result.existing_title}
                  </div>
                )}
              </div>
            ))}
