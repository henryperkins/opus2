...
vi.mock("@monaco-editor/react", () => ({
  default: vi.fn(({ onMount, value, onChange }) => {
    ...
    if (onMount) {
      setTimeout(
        () => onMount(mockEditor, { editor: { createModel: vi.fn() } }),
        0,
      );
    }
...
describe("Monaco Editor Stability Tests", () => {
  ...
  describe("MonacoRoot Component", () => {
    ...
    it("6. Accessibility - has proper ARIA labels", () => {
      render(
        <TestWrapper>
          <MonacoRoot value="test code" filename="test.js" />
        </TestWrapper>,
      );

      expect(
        screen.getByText("Editing test.js in javascript language."),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("ChatLayout Component", () => {
    ...
    it("8. LocalStorage Integration - handles panel state", () => {
      ...
      render(
        <ChatLayout showEditor={true} editor={mockEditor} monacoRef={mockRef}>
          {mockChildren}
        </ChatLayout>,
      );
    ...
    it("9. Responsive Behavior - adapts to mobile", () => {
      ...
      render(
        <ChatLayout showSidebar={true} sidebar={mockSidebar}>
          {mockChildren}
        </ChatLayout>,
      );
      ...
      render(
        <TestWrapper>
          <ChatLayout
            ...
            sidebar={
              <div data-testid="knowledge-assistant">Knowledge Assistant</div>
            }
