...
class AnalyticsService {
  ...
  loadPendingMetrics() {
    try {
      ...
      this.pendingMetrics = metrics.filter(
...
export const analyticsAPI = {
  ...
  trackQuality: async (metrics) => {
    ...
    if (analyticsService.isOnline) {
      try {
        ...
        console.warn(
          "Direct quality tracking failed, queued for later:",
          error,
        );
  ...
  recordFeedback: async (responseId, feedback) => {
    const response = await client.post(
      `/api/analytics/feedback/${responseId}`,
      feedback,
    );
    return response.data;
  },
  ...
  getFlowAnalytics: async (projectId, flowType) => {
    const response = await client.get(
      `/api/analytics/flows/${projectId}/${flowType}`,
    );
    return response.data;
  },
