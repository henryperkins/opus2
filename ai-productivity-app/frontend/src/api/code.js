...
export const codeAPI = {
  async uploadFiles(projectId, files, onProgress) {
    ...
    const response = await client.post(
      `/api/code/projects/${projectId}/upload`,
      formData,
      {
  ...
  async getProjectFiles(projectId, params = {}) {
    const response = await client.get(`/api/code/projects/${projectId}/files`, {
      params,
    });
    return response.data;
  },
  ...
  async reindexProject(projectId) {
    const response = await client.post(
      `/api/code/projects/${projectId}/reindex`,
    );
    return response.data;
  },
  ...
  async getIndexingStatus(projectId) {
    const response = await client.get(
      `/api/code/projects/${projectId}/indexing-status`,
    );
    return response.data;
  },
  ...
  async saveCanvasArtifact(projectId, canvasData) {
    const response = await client.post(
      `/api/code/projects/${projectId}/artifacts/canvas`,
      {
  ...
  async deleteCanvasArtifact(projectId, artifactId) {
    const response = await client.delete(
      `/api/code/projects/${projectId}/artifacts/canvas/${artifactId}`,
    );
    return response.data;
  },
};
