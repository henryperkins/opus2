...
export const modelsAPI = {
  ...
  evaluateCostPerformance: async (models, settings) => {
    return models.map((model) => {
      ...
      return {
        ...
        score: score,
        within_budget:
          !settings.costLimit || estimatedCost <= settings.costLimit,
      };
    });
  },
  ...
  selectOptimalModel: (evaluatedModels, settings) => {
    ...
    return (
      candidates.sort((a, b) => b.score - a.score)[0]?.model_id ||
      settings.default
    );
  },
  ...
  handleFallback: async (failedModel, query, fallbacks) => {
    ...
    throw new Error(
      `All fallback models failed for query after ${failedModel} failed`,
    );
  },
