...
export const validateMessagePayload = (payload) => {
  ...
  if (payload.role && !["user", "assistant", "system"].includes(payload.role)) {
    errors.push(
      `Invalid role: ${payload.role}. Must be one of: user, assistant, system`,
    );
  }
  ...
  ["code_snippets", "referenced_files", "referenced_chunks"].forEach(
    (field) => {
  ...
  if (
    payload.applied_commands &&
    typeof payload.applied_commands !== "object"
  ) {
