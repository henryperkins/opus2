...
export const useChatFlows = (settings = defaultChatSettings) => {
  ...
  if (!isValid && typeof window !== "undefined") {
    ...
    console.warn(
      "Invalid chat settings provided – falling back to defaults",
      errors,
    );
  }
  ...
  const executeKnowledgeFlow = useCallback(
    async (query, projectId) => {
      ...
      try {
        ...
        const analysisResult = await knowledgeAPI.analyzeQuery(
          query,
          projectId,
        );
        ...
        const knowledgeResults = await knowledgeAPI.retrieveKnowledge(
          analysisResult,
          projectId,
          mergedSettings.knowledge,
        );

        setFlowState((prev) => ({
          ...prev,
          knowledgeBase: {
            step: "contextInjection",
        ...
        const contextualizedQuery = await knowledgeAPI.injectContext(
          query,
          knowledgeResults,
          mergedSettings.knowledge,
        );
        ...
        setFlowState((prev) => ({
          ...
          knowledgeBase: {
            step: "modelCall",
            data: contextualizedQuery,
            loading: true,
          },
        }));

        // Step 4: Model Call (delegates to model selection flow)
        const modelResponse = await executeModelSelectionFlow(
          contextualizedQuery,
        ...
        const finalResponse = await knowledgeAPI.addCitations(
          modelResponse,
          knowledgeResults,
          mergedSettings.knowledge.citationStyle,
        );
  ...
  const executeModelSelectionFlow = useCallback(
    async (query, taskType) => {
      try {
        setFlowState((prev) => ({
          ...prev,
        ...
        const capableModels = await modelsAPI.matchCapabilities(
          detectedTask,
          mergedSettings.models,
        );

        setFlowState((prev) => ({
          ...prev,
          modelSelection: {
            step: "costPerformanceEvaluation",
            data: capableModels,
            loading: true,
          },
        }));
        ...
        const evaluatedModels = await modelsAPI.evaluateCostPerformance(
          capableModels,
          mergedSettings.models,
        );
        ...
        setFlowState((prev) => ({
          ...
          modelSelection: {
            step: "modelSelection",
            data: evaluatedModels,
            loading: true,
          },
        }));

        // Step 4: Model Selection
        let selectedModel = modelsAPI.selectOptimalModel(
          evaluatedModels,
          mergedSettings.models,
        );
        ...
        try {
          ...
          setFlowState((prev) => ({
            ...
            modelSelection: {
              step: "complete",
              data: { model: selectedModel, response },
              loading: false,
            },
          }));
          ...
          setFlowState((prev) => ({
            ...prev,
            modelSelection: {
              step: "fallbackHandling",
              data: modelError,
              loading: true,
            },
          }));

          const fallbackResponse = await modelsAPI.handleFallback(
            selectedModel,
            query,
            mergedSettings.models.fallbacks,
          );
  ...
  const executeRenderingFlow = useCallback(
    async (responseData, onStreamUpdate) => {
      try {
        setFlowState((prev) => ({
          ...prev,
          responseRendering: {
            step: "streamReception",
            data: null,
            loading: true,
          },
        }));
        ...
        const streamProcessor = renderingAPI.createStreamProcessor(
          responseData,
          mergedSettings.rendering,
        );

        setFlowState((prev) => ({
          ...prev,
          responseRendering: {
            step: "formatDetection",
        ...
        const formatInfo = await renderingAPI.detectFormats(
        ...
        await streamProcessor.process(async (chunk) => {
          const renderedChunk = await renderingAPI.renderChunk(
            chunk,
            formatInfo,
            mergedSettings.rendering,
          );
        ...
        setFlowState((prev) => ({
          ...prev,
          responseRendering: {
            step: "interactiveElementInjection",
            data: renderedChunks,
            loading: true,
          },
        }));
        ...
        const interactiveElements =
          await renderingAPI.injectInteractiveElements(
            renderedChunks,
            formatInfo,
          );
        ...
        const boundElements =
          await renderingAPI.bindActions(interactiveElements);
