...
class CostTrackingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        try:
            ...
            await self._record_request_metrics(request, response, tracking_context, success=True)
            
            return response
            
        except Exception as e:
            # Record failed request metrics
            await self._record_request_metrics(request, None, tracking_context, success=False, error=str(e))
            raise
    ...
    async def _detect_llm_usage(self, request: Request, response: Optional[Response]) -> bool:
        ...
        if hasattr(request.state, 'body'):
            ...
            if isinstance(body, dict):
                ...
                if any(key in body for key in ["model", "model_id", "prompt", "messages"]):
                    return True
    ...
    async def _extract_model_info(self, request: Request, response: Optional[Response]) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(request.state, 'body'):
                body = request.state.body
                if isinstance(body, dict):
                    model_info["model_id"] = body.get("model", body.get("model_id", "unknown"))
