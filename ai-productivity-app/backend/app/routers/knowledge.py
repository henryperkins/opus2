...
async def get_knowledge_service():
    ...
    if knowledge_service is None:
        try:
            ...
            logger.info(
                "Knowledge service initialized with %s vector backend",
...
async def search_knowledge(
    request: KnowledgeSearchRequest, db: Session = Depends(get_db)
) -> KnowledgeResponse:
    ...
    try:
        ...
        if service:
            ...
            return KnowledgeResponse(success=True, data=response_data.dict())
...
async def search_knowledge_by_project(
    project_id: int, request: KnowledgeSearchRequest, db: Session = Depends(get_db)
) -> KnowledgeResponse:
    """
...
async def build_context(
    request: ContextBuildRequest, db: Session = Depends(get_db)
) -> KnowledgeResponse:
    ...
    try:
        ...
        if service:
            ...
            return KnowledgeResponse(success=True, data=context_result.dict())
        ...
        combined_content = "\n\n".join(
            [f"[{source.title}]\n{source.content}" for source in mock_sources]
        )
...
async def get_knowledge_stats(
    ...
    try:
        ...
        return KnowledgeResponse(success=True, data=stats.dict())
    except Exception as e:
...
async def add_knowledge_entry(
    entry: KnowledgeEntry, db: Session = Depends(get_db)
) -> KnowledgeResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to add knowledge entry: {str(e)}"
        )
...
async def get_knowledge_entry(
    ...
    try:
        ...
        return KnowledgeResponse(success=True, data=mock_entry.dict())
    except Exception as e:
...
async def get_project_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get project summary: {str(e)}"
        ) from e


@router.post("/analyze-query")
async def analyze_query(
    request: QueryAnalysisRequest, db: Session = Depends(get_db)
) -> KnowledgeResponse:
    ...
    try:
        ...
        keywords = [
            word for word in query.split() if word not in stop_words and len(word) > 2
        ][:10]
        ...
        return KnowledgeResponse(success=True, data=analysis.dict())
    except Exception as e:
...
async def retrieve_knowledge(
    request: KnowledgeRetrievalRequest, db: Session = Depends(get_db)
) -> KnowledgeResponse:
    ...
    try:
        ...
        if not service:
            ...
            return KnowledgeResponse(success=True, data=filtered_docs)

        # Use real knowledge service
        project_ids = (
            [int(request.project_id)] if request.project_id.isdigit() else None
        )
        results = await service.search_knowledge(
        ...
        return KnowledgeResponse(success=True, data=entries)
    except Exception as e:
        logger.error(f"Knowledge retrieval failed: {e}")
        raise HTTPException(
            status_code=500, detail=f"Knowledge retrieval failed: {str(e)}"
        )


@router.post("/inject-context")
async def inject_context(
    request: ContextInjectionRequest, db: Session = Depends(get_db)
) -> KnowledgeResponse:
    ...
    try:
        ...
        for i, entry in enumerate(request.knowledge):
            ...
            if current_length + len(context_part) > request.max_context_length:
                ...
                remaining_space = (
                    request.max_context_length - current_length - 20
                )  # buffer
...
async def add_citations(
    request: CitationRequest, db: Session = Depends(get_db)
) -> KnowledgeResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Citation addition failed: {str(e)}"
        )
...
async def _process_knowledge_file(
    ...
    try:
        ...
        session.execute(
            text(insert_sql),
            {
                "document_id": hash(doc_id)
                % 2147483647,  # Convert string ID to int within PostgreSQL int range
                "project_id": doc.project_id,
