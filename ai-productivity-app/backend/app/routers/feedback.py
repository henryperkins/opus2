...
class FeedbackCreate(BaseModel):
    ...
    rating: int = Field(
        ..., ge=-1, le=5
    )  # -1 for thumbs down, 1 for thumbs up, or 1-5 scale
...
async def get_message_feedback(
    ...
    feedback_query = select(UserFeedback).where(UserFeedback.message_id == message_id)
...
async def get_feedback_stats(
    ...
    base_query = select(UserFeedback).where(
        UserFeedback.created_at >= start_date, UserFeedback.created_at <= end_date
    )
    ...
    helpful_percentage = (
        (helpful_count / total_count * 100) if total_count > 0 else None
    )

    accuracy_ratings = [
        f.accuracy_rating for f in feedback_list if f.accuracy_rating is not None
    ]
    accuracy_avg = (
        sum(accuracy_ratings) / len(accuracy_ratings) if accuracy_ratings else None
    )

    clarity_ratings = [
        f.clarity_rating for f in feedback_list if f.clarity_rating is not None
    ]
    clarity_avg = (
    ...
    completeness_ratings = [
        f.completeness_rating
        for f in feedback_list
        if f.completeness_rating is not None
    ]
    completeness_avg = (
        sum(completeness_ratings) / len(completeness_ratings)
    ...
    if rag_responses:
        rag_positive = sum(
            1 for f in rag_responses if f.helpful is True or f.rating > 0
        )
...
async def delete_feedback(
    ...
    feedback = db.execute(
        select(UserFeedback).where(
            UserFeedback.id == feedback_id, UserFeedback.user_id == current_user.id
        )
    ).scalar_one_or_none()
