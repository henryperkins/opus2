...
class UsageSearcher:
    ...
    def __init__(self, project_path: str):
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize Jedi project at {self.project_path}: {e}"
            )
            self.jedi_project = None

    def find_usages(
        self, file_path: str, line: int, column: int, limit: int = 50
    ) -> List[Dict]:
        ...
        try:
            ...
            script = jedi.Script(
                code=source_code, path=full_file_path, project=self.jedi_project
            )
