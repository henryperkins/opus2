...
class CostTrackingService:
    ...
    async def calculate_cost(self, event: UsageEvent) -> CostCalculation:
        ...
        if self._is_cache_valid() and cache_key in self._cost_cache:
            ...
            return CostCalculation(
                input_cost=cached_calc.input_rate_per_1k * Decimal(event.input_tokens) / 1000,
                output_cost=cached_calc.output_rate_per_1k * Decimal(event.output_tokens) / 1000,
                total_cost=cached_calc.input_rate_per_1k * Decimal(event.input_tokens) / 1000 + 
        ...
        model_config = self.db.query(ModelConfiguration).filter(
            ModelConfiguration.model_id == event.model_id,
            ModelConfiguration.provider == event.provider
        ).first()
        
        if not model_config:
            logger.warning(f"Model configuration not found for {event.model_id} ({event.provider})")
    ...
    async def record_usage(self, event: UsageEvent) -> CostCalculation:
        ...
        try:
            ...
            usage_metric = self.db.query(ModelUsageMetrics).filter(
            ...
            if usage_metric.avg_response_time_ms is None:
                ...
                total_time = usage_metric.avg_response_time_ms * (usage_metric.total_requests - 1)
                usage_metric.avg_response_time_ms = (total_time + event.response_time_ms) / usage_metric.total_requests
            ...
            if usage_metric.success_rate is None:
                ...
                total_success = (usage_metric.success_rate / 100.0) * (usage_metric.total_requests - 1)
                if event.success:
                    total_success += 1
                usage_metric.success_rate = (total_success / usage_metric.total_requests) * 100.0
            ...
            if event.feature:
                ...
                if feature_key not in usage_metric.detailed_metrics:
                    usage_metric.detailed_metrics[feature_key] = {"requests": 0, "cost": 0.0}
                usage_metric.detailed_metrics[feature_key]["requests"] += 1
                usage_metric.detailed_metrics[feature_key]["cost"] += float(cost_calc.total_cost)
            
            # Track by user if available
            if event.user_id:
                user_key = f"user_{event.user_id}"
                if user_key not in usage_metric.detailed_metrics:
                    usage_metric.detailed_metrics[user_key] = {"requests": 0, "cost": 0.0}
                usage_metric.detailed_metrics[user_key]["requests"] += 1
                usage_metric.detailed_metrics[user_key]["cost"] += float(cost_calc.total_cost)
    ...
    async def get_usage_aggregates(
        ...
        if user_id:
            ...
            query = query.filter(
                ModelUsageMetrics.detailed_metrics.has_key(user_key)
            )
    ...
    async def get_cost_summary(
        ...
        total_tokens = sum(metric.total_tokens_input + metric.total_tokens_output for metric in metrics)
        ...
        for metric in metrics:
            ...
            model_costs[model_id]["tokens"] += metric.total_tokens_input + metric.total_tokens_output
    ...
    async def get_real_time_usage(self) -> Dict[str, Any]:
        ...
        metrics = self.db.query(ModelUsageMetrics).filter(
            ModelUsageMetrics.period_start == period_start,
            ModelUsageMetrics.period_end == period_end
        ).all()
        ...
        yesterday_metrics = self.db.query(ModelUsageMetrics).filter(
        ...
        if yesterday_requests > 0:
            request_change = ((current_hour_requests - yesterday_requests) / yesterday_requests) * 100
    ...
    async def cleanup_old_metrics(self, days_to_keep: int = 90):
        ...
        deleted_count = self.db.query(ModelUsageMetrics).filter(
            ModelUsageMetrics.period_end < cutoff_date
        ).delete()
