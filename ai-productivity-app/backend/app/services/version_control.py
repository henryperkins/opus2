...
from app.models.document_version import DocumentVersion, DocumentChange, DocumentSnapshot, ChangeBaseline
...
class VersionControlService:
    ...
    async def create_version(
        ...
        try:
            ...
            await self._create_change_records(db, new_version.id, old_content, new_content)
            
            # Create snapshot if this is a major version
            if version_number % self.major_version_threshold == 0:
                await self._create_snapshot(db, document_id, new_version.id, new_content)
    ...
    def _analyze_changes(self, old_content: str, new_content: str) -> Dict[str, Any]:
        ...
        for op, old_start, old_end, new_start, new_end in matcher.get_opcodes():
            if op == 'insert':
                ...
                changes["lines_modified"] += max(old_end - old_start, new_end - new_start)
        
        # Generate summary
        total_changes = changes["lines_added"] + changes["lines_deleted"] + changes["lines_modified"]
        if total_changes == 0:
            ...
            if op == 'insert':
                for i in range(new_start, new_end):
                    change = DocumentChange(
                        ...
                        impact_score=self._calculate_impact_score(new_lines[i] if i < len(new_lines) else "")
                    )
                    db.add(change)
                ...
                for i in range(old_start, old_end):
                    change = DocumentChange(
                        ...
                        impact_score=self._calculate_impact_score(old_lines[i] if i < len(old_lines) else "")
                    )
                    db.add(change)
                ...
                for i in range(max(old_end - old_start, new_end - new_start)):
                    old_line = old_lines[old_start + i] if old_start + i < old_end else ""
                    new_line = new_lines[new_start + i] if new_start + i < new_end else ""
    ...
    def _calculate_impact_score(self, line: str) -> int:
        ...
        if any(pattern in line.lower() for pattern in [
            'def ', 'class ', 'import ', 'from ', 'return', 'raise', 'except'
        ]):
            return 5
        
        # Medium-high impact
        if any(pattern in line.lower() for pattern in [
            'if ', 'elif ', 'else:', 'for ', 'while ', 'try:', 'finally:'
        ]):
            return 4
    ...
    async def _cleanup_old_versions(self, db: Session, document_id: int):
        ...
        if len(versions) > self.max_versions_per_document:
            ...
            for version in versions[self.max_versions_per_document:]:
                ...
                has_snapshot = db.query(DocumentSnapshot).filter(
                    DocumentSnapshot.version_id == version.id
                ).first()
    ...
    async def get_version_diff(
        ...
        from_ver = db.query(DocumentVersion).filter(
            DocumentVersion.document_id == document_id,
            DocumentVersion.version_number == from_version
        ).first()
        
        to_ver = db.query(DocumentVersion).filter(
            DocumentVersion.document_id == document_id,
    ...
    async def _get_version_content(self, db: Session, version_id: int) -> str:
        ...
        snapshot = db.query(DocumentSnapshot).filter(
            DocumentSnapshot.version_id == version_id
        ).first()
        ...
        document = db.query(CodeDocument).filter(
            CodeDocument.id == version.document_id
        ).first()
