...
class ContentFilter:
    ...
    def should_exclude_chunk(
        self, chunk: Dict[str, Any], strict_mode: bool = False
    ) -> bool:
    ...
    def filter_and_validate_chunks(
        self, chunks: List[Dict[str, Any]], strict_mode: bool = False
        ...
        for chunk in chunks:
            ...
            if self.should_exclude_chunk(chunk, strict_mode):
                ...
                warnings.append(
                    f"Excluded chunk from {file_path} due to sensitive content"
                )
                continue
        ...
        if excluded_count > 0:
            warnings.append(
                f"Excluded {excluded_count} chunks due to sensitive content"
            )
