...
class StaticAnalysisSearcher:
    ...
    def run_pylint(self, file_path: str) -> List[Dict]:
        ...
        command = ["pylint", full_file_path, "--output-format=json"]

        try:
            # Run from the project root so pylint can resolve imports correctly
            result = subprocess.run(
                command, capture_output=True, text=True, cwd=self.project_path
            )
            ...
            try:
                ...
                logger.error(
                    f"Pylint produced non-JSON output for {file_path}: {result.stdout or result.stderr}"
                )
                return []
            ...
            logger.error(
                "pylint command not found. Is it installed in the environment?"
            )
            return []
