    ...
    def cleanup_realtime_metrics(self):
        ...
        for metric_name in self.realtime_metrics:
            self.realtime_metrics[metric_name] = [
                metric for metric in self.realtime_metrics[metric_name]
                if metric.timestamp > cutoff_time
            ]
    
    def get_realtime_metrics(self, 
                           metric_names: List[str] = None,
                           time_window: int = 300) -> Dict[str, List[MetricPoint]]:
...
class AnalyticsService:
    ...
    async def track_operation(self, operation_name: str, duration: float, success: bool = True):
    ...
    async def track_rag_performance(self, 
                                   query: str,
                                   response_time: float,
                                   sources_count: int,
    ...
    async def track_search_performance(self,
                                     query: str,
                                     search_type: str,
                                     results_count: int,
                                     response_time: float,
    ...
    async def generate_performance_report(self, 
                                        time_range: Tuple[datetime, datetime]) -> AnalyticsReport:
        ...
        try:
            ...
            performance_metrics = await self._calculate_performance_metrics(db, start_time, end_time)
            
            # Calculate usage metrics
            usage_metrics = await self._calculate_usage_metrics(db, start_time, end_time)
            
            # Calculate quality metrics
            quality_metrics = await self._calculate_quality_metrics(db, start_time, end_time)
            
            # Generate insights
            insights = self._generate_performance_insights(performance_metrics, usage_metrics, quality_metrics)
            
            # Generate recommendations
            recommendations = self._generate_performance_recommendations(performance_metrics, usage_metrics)
    ...
    async def _calculate_performance_metrics(self, 
        ...
        for operation in self.operation_times:
            ...
            if times:
                avg_response_times[operation] = {
                    ...
                    "p95": statistics.quantiles(times, n=20)[18] if len(times) > 10 else max(times),
                    "count": len(times)
                }
        ...
        for operation, error_count in self.error_counts.items():
            total_operations = len(self.operation_times.get(operation, [1]))  # Avoid division by zero
            error_rates[operation] = error_count / total_operations if total_operations > 0 else 0
    ...
    async def _calculate_usage_metrics(self, 
        ...
        active_users = db.query(func.count(func.distinct(ChatMessage.user_id))).filter(
            ChatMessage.created_at.between(start_time, end_time)
        ).scalar() or 0
        
        # Total sessions
        total_sessions = db.query(func.count(ChatSession.id)).filter(
            ChatSession.created_at.between(start_time, end_time)
        ).scalar() or 0
        
        # Messages per user
        avg_messages = db.query(func.avg(func.count(ChatMessage.id))).filter(
            ChatMessage.created_at.between(start_time, end_time)
        ).group_by(ChatMessage.user_id).scalar() or 0
        
        # Document interactions
        document_views = db.query(func.count(CodeDocument.id)).filter(
            CodeDocument.updated_at.between(start_time, end_time)
        ).scalar() or 0
    ...
    async def _calculate_quality_metrics(self, 
        ...
        if feedback_count > 0:
            ...
            helpful_percentage = db.query(
                func.count(UserFeedback.id).filter(UserFeedback.helpful == True)
            ).filter(
                UserFeedback.created_at.between(start_time, end_time)
            ).scalar() / feedback_count * 100
            
            # Quality ratings breakdown
            accuracy_rating = db.query(func.avg(UserFeedback.accuracy_rating)).filter(
                UserFeedback.created_at.between(start_time, end_time),
                UserFeedback.accuracy_rating.isnot(None)
            ).scalar() or 0
            
            clarity_rating = db.query(func.avg(UserFeedback.clarity_rating)).filter(
                UserFeedback.created_at.between(start_time, end_time),
                UserFeedback.clarity_rating.isnot(None)
            ).scalar() or 0
            
            completeness_rating = db.query(func.avg(UserFeedback.completeness_rating)).filter(
                UserFeedback.created_at.between(start_time, end_time),
    ...
    async def _get_database_metrics(self, 
        ...
        try:
            ...
            # Recent activity
            recent_messages = db.query(func.count(ChatMessage.id)).filter(
                ChatMessage.created_at.between(start_time, end_time)
            ).scalar() or 0
    ...
    def _generate_performance_insights(self, 
        ...
        for operation, metrics in response_times.items():
            if metrics["avg"] > 2.0:  # > 2 seconds
                insights.append(f"High response time detected for {operation}: {metrics['avg']:.2f}s average")
        ...
        if avg_rating > 4.0:
            insights.append(f"Excellent user satisfaction: {avg_rating:.1f}/5.0 average rating")
        elif avg_rating < 3.0:
            insights.append(f"User satisfaction needs improvement: {avg_rating:.1f}/5.0 average rating")
        
        confidence_score = quality_metrics.get("content_quality", {}).get("avg_confidence_score", 0)
        if confidence_score < 0.7:
            insights.append(f"RAG confidence may need improvement: {confidence_score:.2f} average confidence")
        
        return insights
    
    def _generate_performance_recommendations(self, 
                                            performance_metrics: Dict[str, Any],
                                            usage_metrics: Dict[str, Any]) -> List[str]:
        ...
        for operation, metrics in response_times.items():
            if metrics["avg"] > 2.0:
                recommendations.append(f"Consider optimizing {operation} - current avg: {metrics['avg']:.2f}s")
        
        # Caching recommendations
        if any(metrics["avg"] > 1.0 for metrics in response_times.values()):
            recommendations.append("Implement or improve caching for frequently accessed data")
        ...
        if total_documents > 10000:
            recommendations.append("Consider database optimization or partitioning for large document set")
    ...
    async def get_realtime_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_error_rate = error_count / total_operations if total_operations > 0 else 0
            
            # User activity
            active_sessions = len([
                session for session in self.user_sessions.values()
                if (datetime.utcnow() - session["last_activity"]).total_seconds() < 300
            ])
