...
class SecurityService:
    ...
    def _calculate_permissions(
        self, user: User, user_roles: Set[str]
    ) -> Set[Permission]:
        """Calculate user permissions based on roles."""
        permissions = set(
            self.security_policies["access_policy"]["default_permissions"]
        )

        if "admin" in user_roles:
            permissions.update(
                self.security_policies["access_policy"]["admin_permissions"]
            )
        ...
        if user_id in self.access_rules:
            for rule in self.access_rules[user_id]:
                if not rule.is_expired():
                    resource_key = (
                        f"{rule.resource_type.value}:{rule.resource_id or '*'}"
                    )
    ...
    async def _check_rate_limit(
        self, context: SecurityContext, resource_type: ResourceType
    ) -> bool:
        ...
        if rate_key not in self.rate_limits:
            self.rate_limits[rate_key] = {"requests": [], "window_start": current_time}
    ...
    async def _check_resource_access(
        self, context: SecurityContext, resource_type: ResourceType, resource_id: str
    ) -> bool:
    ...
    async def _check_project_access(self, user_id: int, project_id: str) -> bool:
        ...
        try:
            ...
            if user_id in self.access_rules:
                for rule in self.access_rules[user_id]:
                    if (
                        rule.resource_type == ResourceType.PROJECT
                        and (rule.resource_id == project_id or rule.resource_id is None)
            ...
            if user_id in self.access_rules:
                for rule in self.access_rules[user_id]:
                    if (
                        ...
                        and (
                            rule.resource_id == document_id or rule.resource_id is None
                        )
                        and not rule.is_expired()
                    ):
                        return True
    ...
    def _check_security_level(
        ...
        if context.security_level == "elevated":
            ...
            if permission in [Permission.DELETE, Permission.ADMIN]:
                ...
                session_age = (
                    datetime.utcnow() - context.authenticated_at
                ).total_seconds()
                max_session_age = 3600  # 1 hour for admin operations

                if session_age > max_session_age:
                    logger.warning(
                        f"Elevated operation denied - session too old: {session_age}s"
                    )
                    return False
    ...
    async def audit_user_access(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "security_summary": {
                    "total_access_attempts": len(
                        [e for e in user_events if "access" in e["event_type"]]
                    ),
                    "failed_attempts": len(
                        [e for e in user_events if e["event_type"] == "access_denied"]
                    ),
