...
def get_tokenizer(model_name: str = "gpt-3.5-turbo"):
    ...
    try:
        ...
        _fallback_mode = True
        logger.warning(
            "tiktoken not available, using fallback token estimation (3.5 chars/token)"
        )
...
def estimate_max_context_tokens(
    text: str, max_context_length: int, model_name: str = "gpt-3.5-turbo"
) -> tuple[str, int]:
    """
...
def validate_token_usage(
    context: str, max_allowed: int, model_name: str = "gpt-3.5-turbo"
) -> dict:
    """
    ...
    return {
        ...
        "utilization_pct": (
            round((token_count / max_allowed) * 100, 1) if max_allowed > 0 else 0
        ),
    }
