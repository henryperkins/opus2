...
async def get_confidence_score(
    ...
    search_results = await knowledge_service.search_knowledge(query, project_ids=[user.current_project_id])
    ...
    try:
        while True:
            ...
            if query:
                ...
                search_results = await knowledge_service.search_knowledge(query, project_ids=[project_id])
                
                if not search_results:
                    await websocket.send_json({"confidence": 0.0, "explanation": "No relevant documents found."})
                else:
                    confidence = confidence_service.calculate_rag_confidence(search_results)
                    explanation = confidence_service.get_confidence_explanation(confidence)
                    await websocket.send_json({"confidence": confidence, "explanation": explanation})
