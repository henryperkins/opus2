...
class AnthropicProvider(LLMProvider):
    ...
    def _initialize_client(self) -> None:
        ...
        self.client = AsyncAnthropic(
            api_key=api_key, timeout=self.config.get("timeout", 300), max_retries=0
        )
    ...
    async def complete(
        ...
        **kwargs,
        ...
        logger.debug(
            f"Anthropic request: model={model}, stream={stream}, thinking={thinking is not None}"
        )
    ...
    async def stream(self, response: Any) -> AsyncIterator[str]:
        ...
        async for chunk in response:
            ...
            if hasattr(chunk, "thinking") and chunk.thinking:
                ...
                thinking_text = getattr(chunk.thinking, "delta", "") or getattr(
                    chunk.thinking, "text", ""
                )
                if thinking_text:
                    yield thinking_text
    ...
    def extract_content(self, response: Any) -> str:
        ...
        if hasattr(response, "thinking") and response.thinking:
            thinking_content = getattr(response.thinking, "content", "") or getattr(
            ...
            if isinstance(response.content, str):
                ...
                for block in response.content:
                    if hasattr(block, "text"):
                        ...
                        ) or getattr(block.thinking, "text", "")
                        if thinking_text:
    ...
    def _supports_thinking(self, model: str) -> bool:
        ...
        thinking_models = {
            m.strip().lower() for m in settings.claude_thinking_models.split(",")
        }
