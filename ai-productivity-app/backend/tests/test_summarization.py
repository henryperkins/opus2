...
class TestSummarizationService:
    ...
    def test_should_summarize_chunks_by_tokens(self):
        ...
        chunks = [
            {
                "content": "This is a medium length piece of content " * 20,  # ~80 tokens each
    ...
    async def test_summarize_overflow_chunks(self):
        ...
        with patch('app.services.summarization_service.llm_client') as mock_client:
            ...
            mock_response.choices[0].message.content = "Summary: Contains function1 and TestClass with basic implementations."
    ...
    async def test_summarize_conversation_history(self):
        ...
        old_messages = [
            ...
            {"role": "assistant", "content": "For database connections, you can use SQLAlchemy..."},
        ]
        ...
        with patch('app.services.summarization_service.llm_client') as mock_client:
            mock_response = MagicMock()
            mock_response.choices = [MagicMock()]
            mock_response.choices[0].message.content = "Previous discussion covered FastAPI app creation and database setup with SQLAlchemy."
            mock_client.complete = AsyncMock(return_value=mock_response)
    ...
    async def test_azure_response_format_handling(self):
        ...
        overflow_chunks = [
            {
                "content": "def azure_test(): pass",
                "metadata": {"file_path": "azure.py"}
            }
        ]
