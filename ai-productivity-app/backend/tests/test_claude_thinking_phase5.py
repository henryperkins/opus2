...
class TestClaudeThinkingImplementation:
    ...
    async def test_claude_thinking_config_construction(self, client):
        ...
        with patch('app.llm.providers.anthropic_provider.AsyncAnthropic') as mock_anthropic:
            ...
            result = await client.complete(
                input=[{"role": "user", "content": "Analyze this complex problem step by step"}],
                max_tokens=500
            )
    ...
    async def test_claude_adaptive_budget_adjustment(self, client):
        ...
        with patch('app.llm.providers.anthropic_provider.AsyncAnthropic') as mock_anthropic:
            ...
            await client.complete(
                input=[{"role": "user", "content": large_prompt}],
                max_tokens=500
            )
    ...
    async def test_azure_tool_streaming_chat_api(self, azure_provider):
        ...
        with patch('openai.AsyncAzureOpenAI') as mock_azure:
            ...
            async def mock_stream():
                ...
                chunk.choices[0].delta.tool_calls[0].function.arguments = '{"expression": "2+2"}'
                yield chunk
    ...
    async def test_tool_streaming_json_parsing_error_handling(self, openai_provider):
        ...
        with patch('openai.AsyncOpenAI') as mock_openai:
            ...
            async def mock_stream():
                ...
                chunk.choices[0].delta.tool_calls[0].function.arguments = '{"incomplete'  # Invalid JSON
                yield chunk
            ...
            full_content = "".join(content_parts)
            assert "[Calling test..." in full_content  # Fallback text for incomplete JSON
...
class TestPhase5Integration:
    ...
    async def test_end_to_end_claude_thinking_flow(self):
        ...
        with patch('app.llm.providers.anthropic_provider.AsyncAnthropic') as mock_anthropic:
            ...
            mock_response.content = [MagicMock(text="Based on my analysis, the answer is...")]
