...
class TestContentFilter:
    ...
    def test_should_exclude_chunk_strict_mode(self):
        ...
        assert content_filter.should_exclude_chunk(high_severity_chunk, strict_mode=True)
        
        # Should not exclude in normal mode (would redact instead)
        assert not content_filter.should_exclude_chunk(high_severity_chunk, strict_mode=False)
    ...
    def test_should_exclude_chunk_high_redaction_ratio(self):
        ...
        assert content_filter.should_exclude_chunk(mostly_secrets_chunk, strict_mode=False)
    ...
    def test_filter_and_validate_chunks(self):
        ...
        filtered_chunks, warnings = content_filter.filter_and_validate_chunks(chunks, strict_mode=True)
        ...
        assert not any("PRIVATE KEY" in chunk.get("content", "") for chunk in filtered_chunks)
        
        # Check that API key was redacted
        config_chunk = next((c for c in filtered_chunks if "config.py" in c.get("metadata", {}).get("file_path", "")), None)
    ...
    def test_get_safe_preview(self):
        ...
        content_with_secret = "API_KEY = 'sk-secret123'\n" + "def function():\n    pass\n" * 20
