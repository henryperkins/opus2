...
@pytest.mark.asyncio
async def test_feedback_creation(async_client: AsyncClient, test_user: User, test_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_feedback_stats(async_client: AsyncClient, test_user: User, test_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_feedback_update(async_client: AsyncClient, test_user: User, test_session: AsyncSession):
...
async def test_confidence_warnings_integration(test_session: AsyncSession):
    ...
    enhanced_response = await processor._add_confidence_warnings(response, context_low_confidence)
    ...
    general_response = "Generally speaking, you should implement this feature carefully."
    enhanced_response = await processor._add_confidence_warnings(general_response, context_no_rag)
    
    assert "based on general knowledge rather than specific documentation" in enhanced_response
    ...
    test_chunks = [
        {
            ...
            "metadata": {"file_path": "safe_file.py"}
        },
        {
