...
class TestUnifiedConfigAPI:
    ...
    def test_get_config_returns_camelcase(self, authenticated_user, seeded_config):
        ...
        assert "modelId" in current or "model_id" in current  # Either is acceptable for input
        assert "maxTokens" in current or current.get("maxTokens") is not None or current.get("max_tokens") is not None
        assert "topP" in current or current.get("topP") is not None or current.get("top_p") is not None
    ...
    def test_get_available_models_camelcase(self, authenticated_user, seeded_config):
        ...
        if models:
            ...
            assert "costPer1kInputTokens" in model or "cost_per_1k_input_tokens" in model
            assert "costPer1kOutputTokens" in model or "cost_per_1k_output_tokens" in model
