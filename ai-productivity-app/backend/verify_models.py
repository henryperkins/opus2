...
def verify_models():
    ...
    with SessionLocal() as session:
        ...
        stmt = select(ModelConfiguration).filter(ModelConfiguration.is_deprecated == True)
        ...
        latest_models = [
            ...
            'claude-3-7-sonnet-20250225', 'claude-3-5-haiku-20241022'
        ]
        ...
        # Check capabilities
        reasoning_models = [m for m in available_models if m.capabilities and m.capabilities.get('supports_reasoning')]
        vision_models = [m for m in available_models if m.capabilities and m.capabilities.get('supports_vision')]
        function_models = [m for m in available_models if m.capabilities and m.capabilities.get('supports_functions')]
